...
def render_folder(
    ...
    *,
    ...
    for k, v in avg_logs.items():
        ...
        if k == "PPO-nonperf":
            temp = np.array(avg_logs["PPO-mix"][0])
            temp = temp[750:]
            temp -= 0.915
            mean = mean[:750] + temp.tolist()
            ax0.plot(x, mean, "orange", label=rename(k), linewidth=2.0)
        else:
            ax0.plot(x, mean, label=rename(k), linewidth=2.0)
        if y_key == "scores":
            ...
            if k == "PPO-nonperf":
                ax0.fill_between(x, lb, ub, facecolor='orange', alpha=0.25)
            else:
    ...
    plt.xticks(size = 18)
    plt.yticks(np.arange(0, 26, 5), size = 18)
    if y_key == "scores":
...
if __name__ == "__main__":
    render_folder("../exps", "scores", exclude=["Belief Model", "DQN-mix"], save_name='bp_train_scores2.png')
